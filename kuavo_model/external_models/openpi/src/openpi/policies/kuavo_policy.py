    ...
    def __call__(self, data: dict) -> dict:
        ...
        inputs = {
            ...
            "image": {
                ...
                "right_wrist_0_rgb": wrist_r_image,
            },
            "image_mask": {
                "base_0_rgb": np.True_,
        ...
        # 透传 needs_flip 标签给模型，以便触发辅助 loss
        # if self.use_flip_aux and "needs_flip" in data:
        #     needs_flip_val = np.asarray(data["needs_flip"], dtype=np.int32)
        #     # 始终squeeze到标量，让_collate_fn正确堆叠成[batch]而非[batch,1]
        #     needs_flip_val = np.squeeze(needs_flip_val)
        #     inputs["needs_flip"] = needs_flip_val
        ...
        # # With prompt augmentation enabled, generate randomized conditional prompts during training
...
@dataclasses.dataclass(frozen=True)
class KuavoOutputsEEF(transforms.DataTransformFn):
    """
    This class is used to convert outputs from the model back the the dataset specific format. It is
    used for inference only.
