...
class LeRobotKuavoDataConfig(DataConfigFactory):
    ...
    # prompt_templates: tuple[str, ...] | None = None

    @override
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        # if self.use_prompt_augmentation and not self.prompt_templates:
        #     raise ValueError("prompt_templates must be set when use_prompt_augmentation=True.")
        # The repack transform is *only* applied to the data coming from the dataset,
        # and *not* during inference. We can use it to make inputs from the dataset look
        # as close as possible to those coming from the inference environment (e.g. match the keys).
        # Below, we match the keys in the dataset (which we defined in the data conversion script) to
...
class LeRobotKuavoRightDataConfig(DataConfigFactory):
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        ...
        data_transforms = _transforms.Group(
            inputs=[kuavo_right_policy.KuavoRightInputs(model_type=model_config.model_type)],
            outputs=[kuavo_right_policy.KuavoRightOutputs()],
        )
  ...
  ## 左手版
@dataclasses.dataclass(frozen=True)
class LeRobotKuavoLeftDataConfig(DataConfigFactory):
    extra_delta_transform: bool = True
    root: str | None = None
    video_backend: str | None = "pyav"
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        ...
        data_transforms = _transforms.Group(
            inputs=[kuavo_left_policy.KuavoLeftInputs(model_type=model_config.model_type)],
            outputs=[kuavo_left_policy.KuavoLeftOutputs()],
        )
